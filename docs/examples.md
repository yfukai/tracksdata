...
attribute-bound colors and sizes, time windows, and exact timestamps) and
`RegionPropsNodes.add_node_attrs` (re-computing region properties from existing masks and an
intensity image) is available at
