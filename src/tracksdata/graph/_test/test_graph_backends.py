...
def test_contract_nodes_basic_functionality(graph_backend: RustWorkXGraph) -> None:
    """Test basic contract_nodes functionality with a simple branching structure."""
    # Skip for SQLGraph backend as it doesn't support contract_nodes
    if isinstance(graph_backend, SQLGraph):
        pytest.skip("contract_nodes is not supported by SQLGraph backend")

    # Create a simple branching structure: A -> B -> D and A -> C -> D
    graph_backend.add_node_attr_key("x", 0.0)

    node_a = graph_backend.add_node({"t": 0, "x": 1.0})
    node_b = graph_backend.add_node({"t": 1, "x": 2.0})
    node_c = graph_backend.add_node({"t": 1, "x": 3.0})
    node_d = graph_backend.add_node({"t": 2, "x": 4.0})

    graph_backend.add_edge_attr_key("weight", 0.0)
    graph_backend.add_edge(node_a, node_b, {"weight": 0.5})
    graph_backend.add_edge(node_a, node_c, {"weight": 0.6})
    graph_backend.add_edge(node_b, node_d, {"weight": 0.7})
    graph_backend.add_edge(node_c, node_d, {"weight": 0.8})

    # Contract to keep only A and D (remove intermediate nodes B and C)
    contracted = graph_backend.contract_nodes([node_a, node_d])

    # Check basic structure - multiple paths create only one edge
    assert contracted.num_nodes == 2
    assert contracted.num_edges == 1  # Both paths A->B->D and A->C->D create one edge A->D

    # Check that A connects to D
    edges_df = contracted.edge_attrs()
    assert len(edges_df) == 1
    assert edges_df[DEFAULT_ATTR_KEYS.EDGE_SOURCE].to_list() == [node_a]
    assert edges_df[DEFAULT_ATTR_KEYS.EDGE_TARGET].to_list() == [node_d]


def test_contract_nodes_edge_cases(graph_backend: RustWorkXGraph) -> None:
    """Test contract_nodes edge cases: empty graph, single node, all nodes."""
    # Skip for SQLGraph backend as it doesn't support contract_nodes
    if isinstance(graph_backend, SQLGraph):
        pytest.skip("contract_nodes is not supported by SQLGraph backend")
    # Test empty graph
    contracted = graph_backend.contract_nodes([])
    assert contracted.num_nodes == 0
    assert contracted.num_edges == 0

    # Test single node
    graph_backend.add_node_attr_key("x", 0.0)
    node = graph_backend.add_node({"t": 0, "x": 1.0})
    contracted = graph_backend.contract_nodes([node])
    assert contracted.num_nodes == 1
    assert contracted.num_edges == 0

    # Test all nodes (should preserve structure)
    node_a = graph_backend.add_node({"t": 1, "x": 2.0})
    node_b = graph_backend.add_node({"t": 2, "x": 3.0})
    graph_backend.add_edge_attr_key("weight", 0.0)
    graph_backend.add_edge(node_a, node_b, {"weight": 0.5})

    contracted = graph_backend.contract_nodes([node, node_a, node_b])
    assert contracted.num_nodes == 3
    assert contracted.num_edges == 1


def test_contract_nodes_preserves_attributes(graph_backend: RustWorkXGraph) -> None:
    """Test that contract_nodes preserves node and edge attributes."""
    # Skip for SQLGraph backend as it doesn't support contract_nodes
    if isinstance(graph_backend, SQLGraph):
        pytest.skip("contract_nodes is not supported by SQLGraph backend")

    graph_backend.add_node_attr_key("x", 0.0)
    graph_backend.add_node_attr_key("label", "default")  # Use non-empty default

    # Create nodes with attributes
    node_a = graph_backend.add_node({"t": 0, "x": 1.0, "label": "A"})
    node_b = graph_backend.add_node({"t": 1, "x": 2.0, "label": "B"})
    node_c = graph_backend.add_node({"t": 2, "x": 3.0, "label": "C"})

    graph_backend.add_edge_attr_key("weight", 0.0)
    graph_backend.add_edge_attr_key("type", "default")  # Use non-empty default

    # Create edges with attributes
    graph_backend.add_edge(node_a, node_b, {"weight": 0.5, "type": "forward"})
    graph_backend.add_edge(node_b, node_c, {"weight": 0.6, "type": "forward"})

    # Contract to keep A and C
    contracted = graph_backend.contract_nodes([node_a, node_c])

    # Check node attributes are preserved
    nodes_df = contracted.node_attrs()
    assert len(nodes_df) == 2

    # Check node A attributes
    node_a_data = nodes_df.filter(nodes_df[DEFAULT_ATTR_KEYS.NODE_ID] == node_a)
    assert len(node_a_data) == 1
    assert node_a_data["x"].to_list() == [1.0]
    assert node_a_data["label"].to_list() == ["A"]

    # Check node C attributes - need to get the mapped node ID
    node_ids = nodes_df[DEFAULT_ATTR_KEYS.NODE_ID].to_list()
    node_c_mapped = next(nid for nid in node_ids if nid != node_a)
    node_c_data = nodes_df.filter(nodes_df[DEFAULT_ATTR_KEYS.NODE_ID] == node_c_mapped)
    assert len(node_c_data) == 1
    assert node_c_data["x"].to_list() == [3.0]
    assert node_c_data["label"].to_list() == ["C"]

    # Check edge attributes are preserved
    edges_df = contracted.edge_attrs()
    assert len(edges_df) == 1
    assert edges_df["weight"].to_list() == [0.6]  # Should preserve the last edge's weight
    assert edges_df["type"].to_list() == ["forward"]


def test_contract_nodes_graphview_backend(graph_backend: RustWorkXGraph) -> None:
    """Test that contract_nodes works with GraphView backend."""
    # Skip for SQLGraph backend as it doesn't support contract_nodes
    if isinstance(graph_backend, SQLGraph):
        pytest.skip("contract_nodes is not supported by SQLGraph backend")
    ...
    node_a = graph_backend.add_node({"t": 0, "x": 1.0})
    node_b = graph_backend.add_node({"t": 1, "x": 2.0})
    node_c = graph_backend.add_node({"t": 2, "x": 3.0})

    graph_backend.add_edge_attr_key("weight", 0.0)
    graph_backend.add_edge(node_a, node_b, {"weight": 0.5})
    graph_backend.add_edge(node_b, node_c, {"weight": 0.6})

    # Create a GraphView (subgraph)
    graph_view = graph_backend.subgraph(node_ids=[node_a, node_b, node_c])

    # Contract the GraphView
    contracted = graph_view.contract_nodes([node_a, node_c])

    # Should have 2 nodes, 1 edge
    assert contracted.num_nodes == 2
    assert contracted.num_edges == 1

    # Check edge structure - need to handle node mapping
    edges_df = contracted.edge_attrs()
    assert len(edges_df) == 1
    assert edges_df[DEFAULT_ATTR_KEYS.EDGE_SOURCE].to_list() == [node_a]
    # Target might be mapped, so just check it's not None
    assert edges_df[DEFAULT_ATTR_KEYS.EDGE_TARGET].to_list() != [None]


def test_contract_nodes_multiple_paths_no_duplicates(graph_backend: RustWorkXGraph) -> None:
    """Test that contract_nodes doesn't create duplicate edges when multiple paths exist."""
    # Skip for SQLGraph backend as it doesn't support contract_nodes
    if isinstance(graph_backend, SQLGraph):
        pytest.skip("contract_nodes is not supported by SQLGraph backend")
    ...
    # Create a structure with multiple paths: A -> B -> D and A -> C -> D
    node_a = graph_backend.add_node({"t": 0, "x": 1.0})
    node_b = graph_backend.add_node({"t": 1, "x": 2.0})
    node_c = graph_backend.add_node({"t": 1, "x": 3.0})
    node_d = graph_backend.add_node({"t": 2, "x": 4.0})
    ...
    graph_backend.add_edge_attr_key("weight", 0.0)
    graph_backend.add_edge_attr_key("path", "")

    # Create two different paths to D
    graph_backend.add_edge(node_a, node_b, {"weight": 0.5, "path": "path1"})
    graph_backend.add_edge(node_b, node_d, {"weight": 0.7, "path": "path1"})
    graph_backend.add_edge(node_a, node_c, {"weight": 0.6, "path": "path2"})
    graph_backend.add_edge(node_c, node_d, {"weight": 0.8, "path": "path2"})

    # Contract to keep only A and D (remove B, C)
    contracted = graph_backend.contract_nodes([node_a, node_d])

    # Should have exactly 2 nodes and 1 edge (not 2 edges)
    assert contracted.num_nodes == 2
    assert contracted.num_edges == 1

    # Check that there's only one edge from A to D
    edges_df = contracted.edge_attrs()
    assert len(edges_df) == 1
    assert edges_df[DEFAULT_ATTR_KEYS.EDGE_SOURCE].to_list() == [node_a]
    assert edges_df[DEFAULT_ATTR_KEYS.EDGE_TARGET].to_list() == [node_d]

    # Verify that edge attributes are preserved (should be from the last edge processed)
    assert edges_df["weight"].to_list() == [0.8]  # From C->D edge
    assert edges_df["path"].to_list() == ["path2"]  # From C->D edge
