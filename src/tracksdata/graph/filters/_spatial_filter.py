...
class SpatialFilter:
    ...
    def __init__(
        ...
        node_ids = np.ascontiguousarray(nodes_df[DEFAULT_ATTR_KEYS.NODE_ID].to_numpy(), dtype=np.uint64)

        self._sg_graph = sg.SpatialGraph(
            ndims=len(attrs_keys),
            node_dtype="uint64",
            node_attr_dtypes={
                "position": f"float32[{len(attrs_keys)}]",
            },
            edge_attr_dtypes={},
            position_attr="position",
            directed=True,
        )
        node_pos = np.ascontiguousarray(nodes_df[attrs_keys].to_numpy(), dtype=np.float32)
