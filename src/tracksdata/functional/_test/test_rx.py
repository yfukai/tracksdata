...
def test_symmetric_branching_path() -> None:
    ...
    assert len(np.unique(tracklet_ids)) == 3  # Three unique track IDs
    assert isinstance(tracks_graph, rx.PyDiGraph)
...
def test_complex_valid_branching() -> None:
    ...
    np.testing.assert_array_equal(tracklet_ids, [1, 2, 2, 2, 3, 4])
    ...
    assert len(tracklet_ids) == 6
    assert len(np.unique(tracklet_ids)) == 4  # {0, {1, 3, 4}, 2, 5}
    assert isinstance(tracks_graph, rx.PyDiGraph)
...
def test_multiple_roots() -> None:
    ...
    assert len(node_ids) == 4
    assert len(tracklet_ids) == 4
    assert len(np.unique(tracklet_ids)) == 2  # Two unique track IDs
    assert isinstance(tracks_graph, rx.PyDiGraph)
